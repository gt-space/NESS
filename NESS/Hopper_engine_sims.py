...
if export_bartz_data:
    ...
    np.savetxt("Hopper_Bartz_HTC_550lbf_2_2_26.csv", data, delimiter=",", header="Y Position (in), HTC (W/m^2-K)", comments="")

if export_gas_temps:
    import numpy as np
    data = np.column_stack((engine.Contour_z, engine.T))
    np.savetxt("Hopper_Gas_Temps_550lbf_2_6_26.csv", data, delimiter=",", header="Y Position (in), Gas Temperature (K)", comments="")
